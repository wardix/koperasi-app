...
describe('SHU Closing', () => {
  ...
  beforeEach(async () => {
    ...
    await db.run("DELETE FROM members WHERE id LIKE 'shu-%'")
    ...
    for (let i = 0; i < 3; i++) {
      ...
      // Create monthly payments in the test year
      for (let m = 0; m < 6; m++) {
        const payDate = `${TEST_YEAR}-${String(m + 1).padStart(2, '0')}-15`;
        await db.run(
  ...
  describe('operating cost synchronization & override', () => {
    it('should use manual biaya_operasional if set for the year', async () => {
      ...
      await db.run(
        `INSERT INTO settings (key, value) VALUES ('biaya_operasional_${TEST_YEAR}', ?) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value`,
        [customCost.toString()]
      )
  ...
  describe('revenue synchronization with accounting journals', () => {
    it('should use actual accounting journal revenue when recorded for the year', async () => {
      ...
      expect(result.realizedPendapatan).toBe(journalRevenueAmount)
  ...
  afterAll(async () => {
    // Clean up all test data generated by this suite
    await db.run("DELETE FROM shu_member_allocations WHERE year IN ('2025', '2026', '2098', '2099') OR memberId LIKE 'shu-%' OR memberId LIKE 'scen-%'")
    await db.run("DELETE FROM shu_closes WHERE year IN ('2025', '2026', '2098', '2099')")
    await db.run("DELETE FROM loan_payments WHERE loanId LIKE 'shu-%' OR loanId LIKE 'scen-%'")
    await db.run("DELETE FROM loan_schedules WHERE loanId LIKE 'shu-%' OR loanId LIKE 'scen-%'")
    await db.run("DELETE FROM loans WHERE id LIKE 'shu-%' OR id LIKE 'scen-%' OR memberId LIKE 'shu-%' OR memberId LIKE 'scen-%'")
    await db.run("DELETE FROM transactions WHERE memberId LIKE 'shu-%' OR memberId LIKE 'scen-%'")
    await db.run("DELETE FROM members WHERE id LIKE 'shu-%' OR id LIKE 'scen-%'")
    await db.run("DELETE FROM settings WHERE key LIKE 'biaya_operasional_%' OR key = 'shu_include_inactive_members'")
  })
})
