...
export default function CashflowTemplate() {
  ...
  const columns: TableColumn<CashflowRow>[] = useMemo(() => [
    ...
    },
    {
      ...
      renderCell: (item: CashflowRow) => (
        <Text type="body" style={{ fontWeight: 500, color: item.flowType === 'inflow' ? 'var(--color-success-500)' : 'var(--color-critical-500)' }}>
              ...
              <Grid columns={{minWidth: 240, repeat: 'fit'}} gap={4}>
                <Card style={{ padding: '20px' }}>
                  <VStack gap={2}>
                    ...
                    <Heading level={2} style={{ color: 'var(--color-success-500)' }}>{formatRp(summary.totalInflow)}</Heading>
                  </VStack>
                </Card>
                <Card style={{ padding: '20px' }}>
                  <VStack gap={2}>
                    <Text type="supporting" color="secondary">Total Arus Keluar</Text>
                    <Heading level={2} style={{ color: 'var(--color-critical-500)' }}>{formatRp(summary.totalOutflow)}</Heading>
                  </VStack>
                </Card>
                <Card style={{ padding: '20px' }}>
                  <VStack gap={2}>
                    <Text type="supporting" color="secondary">Saldo Kas Bersih</Text>
                    <Heading level={2} style={{ color: summary.netCash >= 0 ? 'var(--color-success-500)' : 'var(--color-critical-500)' }}>
